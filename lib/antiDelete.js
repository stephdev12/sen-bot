...
class AntiDelete {
    ...
    async handleRevoke(revocationMessage) {
        try {
            ...
            // Envoyer le rapport (sur le chat actuel pour Sen Bot, comme demandé par la logique responsive)
            await this.socket.sendMessage(original.chatId, { 
                text, 
                mentions: [original.sender, deletedBy] 
            }, { quoted: revocationMessage });
            ...
            if (original.mediaType && fs.existsSync(original.mediaPath)) {
                ...
                if (original.mediaType === 'image') await this.socket.sendMessage(original.chatId, { image: { url: original.mediaPath }, ...mediaOptions });
                else if (original.mediaType === 'video') await this.socket.sendMessage(original.chatId, { video: { url: original.mediaPath }, ...mediaOptions });
                else if (original.mediaType === 'sticker') await this.socket.sendMessage(original.chatId, { sticker: { url: original.mediaPath } });
                else if (original.mediaType === 'audio') await this.socket.sendMessage(original.chatId, { audio: { url: original.mediaPath }, mimetype: 'audio/mp4' });
